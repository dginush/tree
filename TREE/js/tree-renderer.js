const TreeRenderer = (() => {
    ...
    function render() {
        ...
        if (!rid || !ms.length) {
            w.innerHTML = '<div class="empty-state"><div class="icon">🌴</div><h3>בחרו שורש</h3><p>הוסיפו בני משפחה ובחרו שורש לעץ</p></div>';
            return;
        }
        ...
        function build(p, l) {
            ...
            if (ch.length && l < lv) {
                ...
                ch.forEach(c => {
                    if (!vis.has(c.id)) h += '<div class="child-branch">' + build(c, l + 1) + '</div>';
                });
                h += '</div>';
            }
            h += '</div>';
        ...
        function nH(p) {
            ...
            // בדיקת בטיחות - DragConnect אולי לא מוגדר
            const cm = (typeof DragConnect !== 'undefined' && DragConnect.isActive) ? DragConnect.isActive() : false;

            return '<div class="tree-node-content ' + gc + (cm ? ' connectable' : '') + '" data-member-id="' + p.id + '" '
                + (cm ? '' : 'onclick="App.viewMember(\'' + p.id + '\')"')
                + '><div class="tree-node-photo">'
                + (p.photo ? '<img src="' + p.photo + '">' : '<div class="initials" style="background:' + bg + '">' + ini + '</div>')
                + '</div><div class="tree-node-name">' + p.firstName + ' ' + p.lastName + '</div>'
                + '<div class="tree-node-dates">' + (age !== null ? 'גיל ' + age : '') + '</div>'
                + (heb ? '<div class="tree-node-hebrew">🕎 ' + heb + '</div>' : '')
                + '<div class="tree-node-relation">' + (App.RELATION_LABELS[p.relationType] || '') + '</div>'
                + (p.status === 'deceased' ? '<div style="font-size:.68em">🕯️</div>' : '')
                + '</div>';
        }
