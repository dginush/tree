const Auth = (() => {
    ...
    async function onLogin(u) {
        try {
            ...
            if (tree) {
                ...
                // No tree - show tree selection screen
            ...
            // On error, show tree select as fallback
            document.getElementById('appContainer').style.display = 'none';
            document.getElementById('treeSelectScreen').style.display = '';
            document.getElementById('treeSelectUserName').textContent = u.displayName || u.email.split('@')[0];
        }
    ...
    // Global functions for tree selection buttons
    window.createNewTree = async function() {
        ...
        try {
            ...
            const t = await Sharing.createTree(u.uid, u.displayName || u.email);
            ...
            alert('שגיאה ביצירת עץ: ' + e.message);
        }
    };
    ...
    window.joinExistingTree = async function() {
        ...
        try {
            ...
            const r = await Sharing.joinByCode(u.uid, code, u.displayName || u.email);
    ...
    async function login() {
        ...
        try {
            ...
            const m = {
                'auth/user-not-found': 'לא קיים',
                'auth/wrong-password': 'שגוי',
                'auth/invalid-credential': 'שגוי',
